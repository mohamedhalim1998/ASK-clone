...
function Profile() {
  ...
  return (
    <div className="relative w-full">
      {backgroundCover(profileState.coverImageUrl)}
      ...
      <div className="w-full absolute top-0 left-0">
        ...
        <div className="w-2/3 mx-auto text-gray-200">
          {profileBox(
            ...
            !profileState.status,
            () => {
              dispatch(changeStatus(!profileState.status));
            }
          )}
          <div className="flex flex-row">
            <div className="w-2/3">
              <AskQuestionCard label="Ask yourself" showImage />
              {questionTabBar}
              <AnswerCard />
            </div>
            <div className="w-1/3  mx-8">
              <div className="grid grid-cols-2  h-fit">
                <ProfileStats
                  icon="💬"
                  label="Posts"
                  counter={profileState.postsCount}
                />
                <ProfileStats
                  icon="❤️"
                  label="Likes"
                  counter={profileState.likesCount}
                />
                <ProfileStats
                  icon="😎"
                  label="followers"
                  counter={profileState.followersCount}
                />
              </div>
              {aboutMe(
                profileState.bio,
    ...
    <div className="my-auto">
      ...
      <div className="px-4 flex flex-row">
        <div className="flex flex-row justify-between cursor-pointer">
          ...
          <p className="mx-4 text-gray-300 my-auto font-medium">invisible</p>
        </div>
      </div>
    </div>
  </div>
